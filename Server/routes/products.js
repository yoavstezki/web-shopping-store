...
router.get('/list', (req, res, next) => {
    ...
    Product.getAllProducts(err => res.json({'sucsses': false, msg: 'failed to get products list'}),
        products => res.json({products, 'success': true, msg: 'Success to fetch products list'}));
});
...
router.get('/categoryCount', (req, res, next) => {
    ...
    Product.getCategoryCounts(err => res.json({'sucsses': false, msg: 'failed to caount products per category'}),
        callback => res.json({callback, 'success': false, msg: 'Sucsses to count products per category'}));
});
...
router.get('/categoryAvg', (req, res, next) => {
    ...
    Product.getCategoryAvg(err => res.json({'sucsses': false, msg: 'failed to calculate avvarage per category'}),
        callback => res.json({callback, 'success': false, msg: 'Sucsses to calculate avvarage per category'}));
});

router.get('/delete/:serial', (req, res, next) => {
    console.log("Server - post  products/delete");
    Product.deleteProduct(req.params.serial, err => res.json({success: false, msg: 'delete opperation failed'}),
        callback => res.json({success: true, msg: `Success to delete`}));
});
...
router.get('/:serial', (req, res, next) => {
    console.log("Server - get products/list");
    Product.findProduct(req.params.serial, err => res.json({'sucsses': false, msg: 'failed to get product '}),
        product => res.json({product, 'success': true, msg: 'Success to fetch product '}));
});

module.exports = router;

function getProductsOfCategory(categoryName, res) {
    Product.searchProductsOfCategory(categoryName, err => res.json({
            success: false,
            msg: 'Failed to get products of category'
        }),
        callback => res.json({callback, success: true, msg: 'Listing product '}));
}
